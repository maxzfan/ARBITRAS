...
- **Confirm the replay rate reads as intelligible on video.** It is 5 epochs/sec (§5), down from the 10–20 development range because the guide dialogue runs alongside the replay and must stay readable at that pace. Slow further with `--rate 3` if beats 2 and 3 need it. Test this Saturday evening, not Sunday morning.
