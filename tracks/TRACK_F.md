...
## Revision, 2026-09-05 late -- decided while building

1. **Every arc ends on continuity.** The first cut of RECON and COMBAT ended
   on a hold; that reads as a kill switch. Rewritten: two missions correct
   the spoof AUTOMATICALLY, two hand the vehicle to a real person who keeps
   the mission moving, and autonomy returns as trust does.

   | Mission | Mechanism | Why it fits |
   |---|---|---|
   | CASEVAC | **Automatic, terrain-referenced.** Track E (now built: signed OSM pre-map, simulated class sensor, gate check 6). The along-track walk drags the believed position across real land-cover boundaries while the wheel sensor still reads the ground the vehicle is on; each observed class transition pins arc length along the known route. | The failure is along-track; the map has boundaries across the outbound leg; the CASEVAC environment renders the pre-map's real classes as ground zones so the picture is the data |
   | RECON | **Automatic, stationary deduction.** The attack becomes the offset-repeater variant (verified: 118 m displacement; Galileo-only corrected fix within 0.8 m median). At an observation point the scout is stationary by design, so any drag of the fix is the spoof; it anchors on the zero-velocity fix, attributes GPS, drops it, continues on Galileo, and reports the emitter's bearing. | The scout stops to observe anyway; the deduction is itself reconnaissance. Pure meaconing (0.000 m) becomes the README row |
   | LOGISTICS | **Operator takeover without a trusted fix.** Corridor breach withdraws autonomy; a person drives the convoy through the contested stretch by the view; hand-back at DEGRADED. | The corrected fix is withheld (PL 18 m), so the human is the only continuity |
   | COMBAT | **Operator takeover on the Galileo corrected fix** (constellation-first exclusion; PL ~8 m, assumed from the 300 m analog until rerun). Fire-control input reads CONDITIONAL, never TRUSTED on a spoofed fix. | The crude attack saturates three features, so confidence withdraws autonomy while the derived half still stands behind a fix the operator can use |

   The takeover is a page component: keyboard / gamepad / touch drive the
   TRUE vehicle in the presentation frame; the operator view hides the true
...
2. **Page structure.** The console (`index.html`) is under concurrent
   ...
   the routing page is a separate file, `console/web/home.html` (served at
   `/home`, to become `/`), hosting the console in its third section in an
   iframe at `/?mission=<name>`. Hero and selector tiles live in the home
   page; the console integrates the registry (`/mission?name=`, props,
   environment modules) through one call site at integration time.

3. **Environments are modules.** `console/web/env/<name>.js` per mission
   against `console/web/env/CONTRACT.md`, verified in
   `console/web/env/preview.html`; `logistics.js` is the reference (today's
   prairie moved behind the contract). RECON, CASEVAC and COMBAT were
   generated by three agents in parallel. Headless screenshots must be
   judged with `?post=0&shadow=0&envmap=0` (software GL renders the
   composer, shadows and float PMREM near-black).

4. **F-0a is done** (commit a609fb9): with feature 2 wired, the regenerated
   demo transitions NOMINAL → DEGRADED (61) → SURRENDERED (62), recovers
   180/190/200, and surrenders on the credential at 390, exactly as the
   planners predicted. Clean-day FSR at the shipped thresholds is 0.041
