...
`console/arbitras/states.py`, which were fitted to the distribution produced by
...
Explicit values still override, by argument or environment (ARBITRAS_NOMINAL,
ARBITRAS_DEGRADED, ARBITRAS_RESTRICTED), and an override is stamped as
...
ENV = {n: f"ARBITRAS_{n.upper()}" for n in THRESHOLD_NAMES}
...
class MissingThreshold(RuntimeError):
    ...
    ruled values (`ARBITRAS_NOMINAL=` empty is not the same as unset, but
...
def require_thresholds(nominal=None, degraded=None, restricted=None,
    ...
    for name in THRESHOLD_NAMES:
        ...
        raise MissingThreshold(
            ...
            f"console/arbitras/states.py were measured against the superseded "
...
def apply_thresholds(t: Thresholds) -> None:
    ...
    `console/arbitras/states.py` owns the confidence-to-state mechanism and
    ...
    """
    from console.arbitras import states
