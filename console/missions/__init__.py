...
class Mission:
    ...
    # the operator-facing rewrite of `beats`, one entry per CHECKPOINT, each
    # carrying 1-3 lines. A checkpoint is `{"epoch", "kind", "label", "lines"}`:
    #   kind "waypoint" -- a prop the operator watches the vehicle reach, at
    #     the epoch it ARRIVES: the first epoch accrued progress reaches the
    #     prop's arc length, `s += speed_m_per_epoch * gain[state]` per epoch,
    #     computed by dialogue.waypoint_checkpoints(mission, mission_states(m)).
    #     Not arc / speed: every mission halts or slows under attack, so the
    #     unimpeded schedule fires while the vehicle is still short of the prop
    #     (DIALOGUE.md revision 2, CORRECTED). Labelled with the prop's own
    #     label, and worded as arrival, because the vehicle IS there;
    #   kind "event"    -- something the stream did (a beat, a credential
    #     change) or the line that closes the run, labelled by what happened.
    # An event checkpoint may NOT be moved onto a waypoint: its `path` claims
    # are verified against the epoch it is emitted on and the same number is not
    # true later, so a moved line is suppressed at runtime and lost in silence.
    # Every numeral in a line's `text` is covered by a claim holding either a
    # contract `path` (dug out of the epoch and verified before display, as
    # explain.verify does) or a named `source` (a scenario parameter, which is
    # not a measurement). design.md §14.
...
LOGISTICS = Mission(
    ...
    guide=(
        # The operator-facing rewrite of `beats`: event entries on the beat's
        # own epoch, waypoint entries on the epoch the convoy ARRIVES.
        ...
        {"epoch": 0, "kind": "waypoint", "label": "FOB · SUPPLY POINT", "lines": [
        ...
        {"epoch": 60, "kind": "event", "label": "carry-off begins", "lines": [
        ...
        {"epoch": 65, "kind": "event", "label": "authority steps down", "lines": [
        ...
        {"epoch": 67, "kind": "event", "label": "corridor breach", "lines": [
        ...
        {"epoch": 150, "kind": "event", "label": "spoofer stops", "lines": [
        ...
        {"epoch": 190, "kind": "event", "label": "hand-back", "lines": [
        ...
        {"epoch": 197, "kind": "waypoint", "label": "PL AMBER", "lines": [
            {"text": "The convoy is at PL AMBER. It reached the phase line late, having spent the contested stretch halted and then in a person's hands, and it is driving itself again, one step short of full authority.",
             "tone": "good", "claims": []},
            {"text": "The fix carrying it across is sound: full sky, nothing excluded, confidence 0.93, and 2.7 m of protection level against the 15 m corridor.",
             "tone": "good",
             "claims": [{"text": "0.93", "value": 0.9253, "path": "confidence"},
                        {"text": "2.7 m", "value": 2.71, "path": "geometry.correction.protection_level_m"},
                        {"text": "15 m", "value": 15.0, "path": "geometry.correction.alert_limit_m"}]},
        ]},
        {"epoch": 200, "kind": "event", "label": "authority restored", "lines": [
        ...
        {"epoch": 333, "kind": "waypoint", "label": "RP KILO · RESUPPLY POINT", "lines": [
            {"text": "The convoy is at the resupply point, the last thing on the route, and the fix that brought it in is sound: full sky, nothing excluded, protection level 3.8 m against the 15 m corridor.",
             "tone": "good",
             "claims": [{"text": "3.8 m", "value": 3.79, "path": "geometry.correction.protection_level_m"},
                        {"text": "15 m", "value": 15.0, "path": "geometry.correction.alert_limit_m"}]},
            {"text": "One thing has changed quietly: the key for the next window has not been disclosed, so the credential reads PENDING. Nothing is overridden yet, and the last authorisation still holds.",
             "tone": "calm", "claims": []},
        ]},
        {"epoch": 390, "kind": "event", "label": "authorisation expired", "lines": [
        ...
        {"epoch": 480, "kind": "event", "label": "window still shut", "lines": [
            {"text": "The window is still shut and no key has arrived. Confidence is 0.90, the whole sky is trusted, and the convoy has not moved: a clean sky does not buy authority back on its own.",
             "tone": "bad",
             "claims": [{"text": "0.90", "value": 0.8975, "path": "confidence"}]},
            {"text": "That is the shape of having two votes. The signal half says go, the credential half says no, and the vehicle obeys whichever of them is the stricter.",
             "tone": "calm", "claims": []},
        ]},
    ),
    behaviour={
...
RECON = Mission(
    ...
    guide=(
        ...
        {"epoch": 0, "kind": "waypoint", "label": "PATROL BASE", "lines": [
        ...
        {"epoch": 60, "kind": "event", "label": "repeater on", "lines": [
        ...
        {"epoch": 61, "kind": "event", "label": "anchor held", "lines": [
        ...
        {"epoch": 70, "kind": "event", "label": "emitter located", "lines": [
        ...
        {"epoch": 84, "kind": "event", "label": "authority restored", "lines": [
        ...
        {"epoch": 90, "kind": "event", "label": "repeater off", "lines": [
        ...
        {"epoch": 100, "kind": "event", "label": "OP-1 report filed", "lines": [
        ...
        {"epoch": 104, "kind": "event", "label": "full sky", "lines": [
        ...
        {"epoch": 147, "kind": "waypoint", "label": "OP-2", "lines": [
            {"text": "The scout is at the second observation point, and the sky is behaving: full sky, nothing excluded, confidence 0.88. This report files clean, and the tag on it says so.",
             "tone": "calm",
             "claims": [{"text": "0.88", "value": 0.8765, "path": "confidence"}]},
        ]},
        {"epoch": 176, "kind": "waypoint", "label": "OP KESTREL", "lines": [
            {"text": "The scout is at the observation post. Nothing has been excluded since the repeater went dark, and an attacker starting now would have 10.7 m of room before the ranges disagreed.",
             "tone": "calm",
             "claims": [{"text": "10.7 m", "value": 10.7, "path": "geometry.displacement_bound_m"}]},
        ]},
        {"epoch": 270, "kind": "event", "label": "credential pending", "lines": [
            {"text": "The message authorising the next window has arrived; the key that proves it has not been disclosed yet, so the credential reads PENDING. Confidence is 0.86 and authority does not move.",
             "tone": "calm",
             "claims": [{"text": "0.86", "value": 0.8601, "path": "confidence"}]},
            {"text": "Pending is normal and it lasts exactly the disclosure lag. I say it out loud because the operator should see the credential channel working, not only see it fail.",
             "tone": "calm", "claims": []},
        ]},
        {"epoch": 344, "kind": "waypoint", "label": "OP-3", "lines": [
            {"text": "The scout is at the last observation point of the loop. The fix rests on 16 satellites with 2 excluded, and carries 3.7 m of protection level against the 100 m a grid-square report is held to.",
             "tone": "good",
             "claims": [{"text": "16", "value": 16, "path": "geometry.correction.trusted_count"},
                        {"text": "2", "value": 2, "path": "geometry.excluded_sv"},
                        {"text": "3.7 m", "value": 3.73, "path": "geometry.correction.protection_level_m"},
                        {"text": "100 m", "value": 100.0, "path": "geometry.correction.alert_limit_m"}]},
        ]},
        {"epoch": 390, "kind": "event", "label": "authorisation renewed", "lines": [
        ...
        {"epoch": 461, "kind": "event", "label": "loop complete", "lines": [
...
CASEVAC = Mission(
    ...
    guide=(
        ...
        {"epoch": 0, "kind": "waypoint", "label": "ROLE 1 AID STATION", "lines": [
        ...
        {"epoch": 62, "kind": "event", "label": "ground disagrees", "lines": [
        ...
        {"epoch": 84, "kind": "event", "label": "false arrival refused", "lines": [
        ...
        {"epoch": 120, "kind": "event", "label": "bound from the ground", "lines": [
        ...
        {"epoch": 150, "kind": "event", "label": "authority returns", "lines": [
        ...
        {"epoch": 158, "kind": "event", "label": "pin re-zeroes", "lines": [
        ...
        {"epoch": 170, "kind": "event", "label": "full authority", "lines": [
        ...
        {"epoch": 264, "kind": "event", "label": "arrival confirmed", "lines": [
        ...
        {"epoch": 275, "kind": "waypoint", "label": "CCP · CASUALTY COLLECTION POINT", "lines": [
            {"text": "The vehicle is at the collection point. The terrain pin puts it 0.2 m from the ring, and that range was counted off the ground the wheels crossed, not off the satellites.",
             "tone": "good",
             "claims": [{"text": "0.2 m", "value": 0.23, "path": "terrain.route_fix.ccp_range_m"}]},
            {"text": "Confidence is 0.87 and the sky has been clean since the spoofer stopped, but arrival still rests on the ground: ranges agreeing on a place is not the same as it being the right place.",
             "tone": "calm",
             "claims": [{"text": "0.87", "value": 0.8659, "path": "confidence"}]},
        ]},
        {"epoch": 290, "kind": "event", "label": "casualty aboard", "lines": [
...
COMBAT = Mission(
    ...
    guide=(
        ...
        {"epoch": 0, "kind": "waypoint", "label": "LD", "lines": [
        ...
        {"epoch": 60, "kind": "event", "label": "spoofer on", "lines": [
        ...
        {"epoch": 61, "kind": "event", "label": "operator drives", "lines": [
        ...
        {"epoch": 80, "kind": "event", "label": "spoofer off", "lines": [
        ...
        {"epoch": 120, "kind": "event", "label": "authority restored", "lines": [
        ...
        {"epoch": 130, "kind": "event", "label": "fire control trusted", "lines": [
        ...
        {"epoch": 209, "kind": "waypoint", "label": "PL RED", "lines": [
            {"text": "The vehicle is at PL RED, and the crossing goes out on a position I stand behind: 19 satellites trusted, protection level 2.6 m against the 15 m a target grid is held to.",
             "tone": "good",
             "claims": [{"text": "19", "value": 19, "path": "geometry.correction.trusted_count"},
                        {"text": "2.6 m", "value": 2.62, "path": "geometry.correction.protection_level_m"},
                        {"text": "15 m", "value": 15.0, "path": "geometry.correction.alert_limit_m"}]},
            {"text": "Since the baselines caught up nothing has been excluded and the geometry has stayed whole. That is what this run looks like when nobody is attacking it.",
             "tone": "calm", "claims": []},
        ]},
        {"epoch": 270, "kind": "event", "label": "credential pending", "lines": [
            {"text": "The message authorising the next window is here, the key that proves it is not, so the credential reads PENDING. Confidence 0.87 and a full sky: nothing is overridden, nothing is assumed.",
             "tone": "calm",
             "claims": [{"text": "0.87", "value": 0.8695, "path": "confidence"}]},
        ]},
        {"epoch": 332, "kind": "waypoint", "label": "OBJ HAWK", "lines": [
            {"text": "The vehicle is on the objective. Fire-control input reads TRUSTED, protection level 3.8 m, and an attacker starting now would have 11.0 m of room before the ranges disagreed.",
             "tone": "good",
             "claims": [{"text": "3.8 m", "value": 3.79, "path": "geometry.correction.protection_level_m"},
                        {"text": "11.0 m", "value": 11.0, "path": "geometry.displacement_bound_m"}]},
            {"text": "Every target grid this vehicle sends starts from its own position, so that room is not an abstraction. It is error that would land in somebody else's grid square.",
             "tone": "good", "claims": []},
        ]},
        {"epoch": 344, "kind": "event", "label": "objective consolidated", "lines": [
        ...
        {"epoch": 480, "kind": "event", "label": "end of window", "lines": [
            {"text": "The objective is held and the credential is current again. Confidence 0.87, full sky, nothing excluded: the run ends on the same footing it started on.",
             "tone": "good",
             "claims": [{"text": "0.87", "value": 0.8664, "path": "confidence"}]},
            {"text": "Nothing was shut down and nothing was taken on trust. Where the fix could not be certified a person drove, and the grids that went out in that stretch went out marked conditional.",
             "tone": "good", "claims": []},
        ]},
    ),
    behaviour={
